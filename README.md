...
`pages deploy` depo kökünden koşulur; böylece `functions/` ve `wrangler.toml` (Pages ayarları, tek kaynak) birlikte yüklenir. Geribildirim maili token'sızdır: fonksiyon `MAILER` servis bağlantısıyla dışa kapalı `zanes-geribildirim-mailer` Worker'ına gider; onun e-posta bağlantısı yalnız geribildirim@zanesiletisim.info → burakaksoy@zanes.com.tr gönderebilir (geçici alıcı, Burak kararı 2026-09-26; info@ kutusu erişilebilir olunca oraya döner: alıcı `functions/api/geribildirim.js` `TO` ve postacı `destination_address` birlikte değişir, adres önce doğrulanmış hedef adres olmalı, test ikisinin eşleşmesini kontrol eder). Postacı değişirse `npx wrangler@4.132.0 deploy --config workers/mailer/wrangler.jsonc` (Workers yazma izni ister; varsayılan dar oturumda yoktur). Ön koşul (bir kez): `zanesiletisim.info`'da Email Routing açık (bu hesapta Email Sending ürünü kapalı; doğrulanmış tek adrese gönderim Routing ile çalışır), alıcı doğrulanmış hedef adres. Doğrulanmış adrese gönderim her planda ücretsizdir. Postacıya ulaşılamazsa form 503 döner ve ziyaretçiye telefon önerilir.
...
- Tek form "Ziyaretiniz nasıldı?" geribildirimidir: puan, mağaza, konu ve isteğe bağlı yorum `/api/geribildirim` adresine gider (JavaScript ile JSON, olmadan düz form). `functions/api/geribildirim.js` (Pages Function) cevabı doğrular ve dışa kapalı postacı Worker'ı (`workers/mailer`, Cloudflare Email Service bağlantısı) üzerinden şimdilik burakaksoy@zanes.com.tr'ye mail atar (ileride info@); 1-2 puan konu satırında "Düşük puan" diye başlar. Kişisel bilgi alanı yoktur; botlar için gizli `website` alanı ve aynı-site (Origin) kontrolü vardır.
